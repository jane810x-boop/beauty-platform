...
// Pattern 1: zh: 'value' (no quotes around zh)
const singleValueRegex = /\bzh:\s*'((?:[^'\\]|\\.)*)'/g;
let singleMatch;
let singleInserts = 0;
...
// Pattern 3: zh: { ... } object (for content blocks)
// This is tricky - need to handle multi-line objects
// For now, let's skip and handle separately
...
// Now handle zh: { ... } object blocks separately
// These are multi-line, so we need to find them carefully
console.log('\n--- Checking zh object blocks ---');

// Find all zh: { ... } where ... contains Chinese
const objectRegex = /\bzh:\s*\{/g;
let objectMatch;
let objectCount = 0;
...
console.log(`Found ${objectCount} zh object blocks (need manual zh-HK addition)`);
